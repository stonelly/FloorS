...
namespace Hartalega.FloorSystem.Windows.UI.HourlyBatchCard
{
    ...
    public delegate void PrintAsyncON2G(List<BatchDTO> lstBatchDTO);
    public partial class GloveBatchOrder : FormBase
    {
        ...
        private void dgvLineDetails_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (dgvLineDetails.Rows[e.RowIndex].Cells[8].Value.ToString() == "Reprint")
                {
                    ...
                    if (!string.IsNullOrEmpty(_passwordForm.Authentication))
                    {
                        ...
                        ReprintHBC _frmManualPrint = new ReprintHBC(serialNumber, resource);
                ...
                {
                    GlobalMessageBox.Show(Messages.REPRINT_CHANGEGLOVETYPE, Messages.CONTACT_MIS, GlobalMessageBoxButtons.OK, SystemIcons.Warning);
                }
            }
        }
        ...
        private void getLineSelection(List<GloveBatchOrderDTO> dtBatchOrderDetails = null)
        {
            try
            {
                if (dtBatchOrderDetails == null)
                {
                    ...
                    dtBatchOrderDetails = HourlyBatchCardBLL.GetBatchOrderDetails("Glove", plantNo);
        ...
        private void getLineSelectionDetails(string batchOrderNo)
        {
            try
            {
                ...
                dtBatchOrderPrintDetails = HourlyBatchCardBLL.GetBatchOrderPrintDetails(batchOrderNo);
