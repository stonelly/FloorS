// -----------------------------------------------------------------------
// <copyright file="RejectGlove.cs" company="Avanade">
...
namespace Hartalega.FloorSystem.Windows.UI.QCScanning
{
    ...
    /// File Type: Code file
    /// </summary> 
    public partial class RejectGlove : FormBase
    {
        ...
        /// </summary>
        public RejectGlove(string moduleName)
        {
            InitializeComponent();
        ...
        private void RejectGlove_Load(object sender, System.EventArgs e)
        {
        ...
        private void GetLineShiftAndReason()
        {
            ...
            {
                // Bind line data
                cmbBoxLine.DataSource = CommonBLL.GetAllLinesByLocation(WorkStationDTO.GetInstance().Location);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (validateRequiredFields())
            {
                ...
                {
                    txtGloveType_Leave(this, new EventArgs());
                    txtGloveType_Validated(this, new EventArgs());
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtGloveType_Validated(object sender, EventArgs e)
        {
        ...
        private void GetGloveDescriptionIfValid()
        {
            ...
            try
            {
                if (txtGloveType.Text != String.Empty)
                {
                    gloveDescription = Convert.ToString(TumblingBLL.GetRejectGloveType(txtGloveType.Text.Trim()));
                    if (gloveDescription != String.Empty && gloveDescription != null)
                    {
                        txtGloveDescription.Text = gloveDescription;
                        if (_previousGloveType != txtGloveType.Text)
                        {
                            txtBatchWeight.Text = String.Empty;
                            cmbBoxReason.SelectedIndex = -1;
                            txtBatchWeight.Focus();
                    ...
                    {
                        GlobalMessageBox.Show(Messages.INVALID_GLOVETYPE_REJECT, Constants.AlertType.Warning, Messages.INVALID_DATA, GlobalMessageBoxButtons.OK);
        ...
        private void SaveBatchCard(string authorizedBy)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(txtGloveDescription.Text))
                {
                    ...
                    objBatchDTO.GloveTypeDescription = txtGloveDescription.Text.ToString();// + Environment.NewLine + Constants.TAB + _GloveCategory; 20220204 Navan: HSB only required GloveTypeDescription in Print reject sticker
                }
                else
                    objBatchDTO.GloveTypeDescription = String.Empty;
                CommonBLL.PrintDetails(Constants.TIME + batchDateTime.ToLongTimeString(), _resultDTO.SerialNumber, Constants.BATCH_TEXT + _resultDTO.BatchNumber, Constants.BATCH_WEIGHT + objBatchDTO.BatchWeight.ToString(), String.Empty, String.Empty, false, objBatchDTO.GloveTypeDescription, String.Empty, Constants.REJECT_GLOVE_SCREEN, false, _chckBatchWeight);
            }
            catch (FloorSystemException ex)
            {
                ExceptionLogging(ex, _screenName, _className, "SaveBatchCard", txtGloveDescription.Text, cmbBoxLine.Text, txtBatchWeight.Text);
                return;
            }
        }
