...
{
    public partial class PrintReproductionVisualTestBatchCard : FormBase
    {
        #region Static Variables
        public static BatchDTO _resultDTO;
        public static string _previousGloveType;
        private static string _screenName = "Tumbling - PrintReproductionVisualTestBatchCard";
        private static string _className = "PrintReproductionVisualTestBatchCard";
        private static bool _chckTenPcs = false;
        private static bool _chckBatchWeight = false;
        #endregion
        #region Load Page
        public PrintReproductionVisualTestBatchCard()
        {
            InitializeComponent();
        ...
        private void PrintReproductionVisualTestBatchCard_Load(object sender, System.EventArgs e)
        {
            try
            {
                GetVTType();
            ...
            {
                ExceptionLogging(ex, _screenName, _className, "PrintVisualTestBatchCard_Load", null);
                return;
            }
        }
        #endregion
        ...
        private void GetVTType()
        {
            try
            {
                //Bind VisualTest Area 
                cmbBoxType.DataSource = CommonBLL.GetEnumText(Constants.VISUAL_TEST_TYPE);
            ...
            if (txtGloveType.Text != String.Empty && txtGloveDescription.Text != String.Empty && cmbBoxSize.DataSource != null)
            {
                if (sizeName != String.Empty)
                {
                    try
                    {
                        ...
                        //get line by glovetype and size
                        GetLineByGloveType();
                    }
                    catch (FloorSystemException ex)
                    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                validateResult = CommonBLL.ValidateGloveTypeSizeAndLine(cmbBoxLine.Text, cmbBoxSize.Text, txtGloveDescription.Text, true);
                validateResult = string.Empty; //bypass 2nd validation
            }
            catch (FloorSystemException ex)
            {
        ...
        private void GetLineByGloveType()
        {
            try
            {
                // Bind Size Data
                
                //Azrul HSB SIT2 Issue: To filter LineNo by GloveType.
                cmbBoxLine.DataSource = CommonBLL.GetLineByGloveTypeTumbling(WorkStationDTO.GetInstance().Location,txtItemId.Text, cmbBoxSize.Text.ToString());
                cmbBoxLine.DisplayMember = "DisplayField";
                cmbBoxLine.SelectedIndex = -1;
                cmbBoxLine.Focus();
            }
            ...
            {
                ExceptionLogging(ex, _screenName, _className, "GetLineByGloveType", txtGloveDescription.Text);
            }
        }
        ...
        private void SaveBatchCard(string authorizedBy, int authorizedFor)
        {
            ...
            objBatchDTO.SubModule = WorkStationDTO.GetInstance().SubModule; //20211209: Azrul HSB SIT Issue - Merged from NGC
            ...
            try
            {
                ...
                CommonBLL.PrintDetails(batchDateTime.ToString("HH:mm:ss"), _resultDTO.SerialNumber,
                     _resultDTO.BatchNumber, objBatchDTO.BatchWeight.ToString(), objBatchDTO.Size,
                     objBatchDTO.TenPcsWeight.ToString(), lblTenPcsWeight.Visible, objBatchDTO.GloveTypeDescription,
        ...
        private void GetTenPcsWeight()
        {
            try
            {
                lblTenPcsWeight.Visible = true;
                lblTenPcsWeight.Text = Constants.CONNECTING;
        ...
        private void ValidateTenPcsWeight()
        {
            ...
            try
            {
                if (txtGloveType.Text != string.Empty && cmbBoxSize.Text != string.Empty)
                    weight = CommonBLL.GetMinMaxTenPcsWeightTumbling(txtItemId.Text, cmbBoxSize.Text);
            }
            catch (FloorSystemException ex)
            {
                ExceptionLogging(ex, _screenName, _className, "validateTenPcsWeight", txtGloveDescription.Text, cmbBoxSize.Text);
                return;
            }
            if (Convert.ToBoolean(WorkStationDataConfiguration.GetInstance().LWeight) && weight.MinWeight != null)
            {
                if (Convert.ToBoolean(WorkStationDataConfiguration.GetInstance().LWeight) && (Convert.ToDouble(txtTenPcsWeight.Text) < Convert.ToDouble(weight.MinWeight) || Convert.ToDouble(txtTenPcsWeight.Text) > Convert.ToDouble(weight.MaxWeight)))
                {
                    ...
                    lblTenPcsWeight.Visible = true;
                    lblTenPcsWeight.Text = Constants.WEIGHT_OUT_OF_RANGE;
                    _chckTenPcs = true;
                }
                else
                {
        ...
        private void GetBatchWeight()
        {
            ...
            {
                ExceptionLogging(ex, _screenName, _className, "getBatchWeight", String.Empty);
                return;
            }
            lblBatchWeight.Visible = false;
        }
        ...
        {
            if (Convert.ToBoolean(WorkStationDataConfiguration.GetInstance().LWeight) & true & (Convert.ToDouble(txtBatchWeight.Text) < Convert.ToDouble(FloorSystemConfiguration.GetInstance().MinBatchWeight) || Convert.ToDouble(txtBatchWeight.Text) > Convert.ToDouble(FloorSystemConfiguration.GetInstance().MaxBatchWeight)))
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void cmbBoxShift_Validated(object sender, EventArgs e)
        {
            if (cmbBoxShift.Text != String.Empty)
            {
                if (txtTenPcsWeight.Text == String.Empty)
                    txtTenPcsWeight.Focus();
            }
