...
{
    public partial class WorkOrderStatusList : FormBase
    {
        #region Member Variables

        private string _screenName = "Work Order - Status";
        ...
        public WorkOrderStatusList(string loggedInUser)
        {
            InitializeComponent();
        ...
        private void dgvLineSelection_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > Constants.MINUSONE)
            {
                if (dgvLineSelection.Rows.Count > 0 && dgvLineSelection.Rows[e.RowIndex] != null)
                {
                    ...
                    new WorkOrderStatusView(_workOrderDTO, _loggedInUser).ShowDialog();
                    PopulateGrid();
                }
            }
        }
        ...
        private void BindItem()
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                if (item.Name == Constants.OPEN || item.Name == Constants.APPROVED)
                {
        ...
        private void PopulateGrid()
        {
            try
            {
                ...
                _dtWorkOrderList = WorkOrderBLL.GetDTWorkOrderList(workorderstatus,
                    txtOrderNo.Text, txtCustomer.Text, txtCustomerPO.Text, txtCustomerRef.Text, OrderStartDate,
                    OrderEndDate, ETDStartDate, ETDEndDate);
