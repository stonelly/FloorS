...
{
    public partial class ScanMultipleBatchMTS : FormBase
    {
        ...
        private void txtSerialNumber_Leave(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtSerialNumber.Text))
            {
                ...
                else if (QAIBLL.GetLastServiceName(Convert.ToDecimal(txtSerialNumber.Text.Trim())) == ReworkOrderFunctionidentifier.RWKCR)
                {
        ...
        private void GetActivePOList()
        {
            try
            {
                List<DropdownDTO> lstFirstGradeActivePOList = FinalPackingBLL.GetFirstGradePOMTSList().Distinct<DropdownDTO>().ToList();
                                ...
                                if (rowsaffected > Constants.ZERO)
                                {
                                    ...
                                    while (palletCasesCount > Constants.ZERO)
                                    {
                                        if (palletCasesCount > _palletcapacity - selectedPalletCasesPlaced)
                                        {
                                            ...
                                            //InsertDataToEWNTable(_palletid, false, objFinalPackingDTO); //20221001: HSB SIT2 Issue# 36: MTS pallet not require insert record into EWN_CompletedPallet table.
                                            //end edit (17Aug2017)
                                    ...
                                    while (preshipmentPalletCasesCount > Constants.ZERO)
                                    {
                                        if (preshipmentPalletCasesCount > _palletcapacity - selectedPreshipPalletCasesPlaced)
                                        {
                                            ...
                                            //InsertDataToEWNTable(_preshipmentPltID, true, objFinalPackingDTO); //20221001: HSB SIT2 Issue# 36: MTS pallet not require insert record into EWN_CompletedPallet table.
                                            //end edit (17Aug2017)
                                            PalletSelection _PalletSelectionForm = new PalletSelection();
